...
A lista atual (ao vivo na Scryfall) é anexada automaticamente ao final da
resposta de `info_brackets`, então **não há lista fixa neste arquivo** — assim
nunca desatualiza. Para checar uma lista de cartas, use `verificar_game_changers`.

Observação útil: **Sol Ring NÃO é Game Changer** (engano comum). Na dúvida sobre
qualquer carta, confirme via `is:gamechanger` na Scryfall — nunca de memória.
